...
public class StrategyAnalysisService {
    ...
    @Transactional
    public void modifyDailyAnalysis(Long strategyId, TraderDailyAnalysisRequestDto analysisRequests) {
        Strategy strategy = findStrategyById(strategyId);
        ...
        DailyAnalysis dailyAnalysis = dailyAnalysisRepository.findDailyAnalysisByStrategyAndDate(
                        strategy,
                        analysisRequests.getDate())
                .orElseThrow(() -> new BusinessException(ErrorCode.DAILY_ANALYSIS_NOT_FOUND));

        dailyAnalysis.modifyDailyAnalysis(analysisRequests.getTransaction(), analysisRequests.getDailyProfitLoss());
    }
